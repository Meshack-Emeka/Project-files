...
hoslem.test(as.numeric(glm.forward$y), fitted(glm.forward)) # the test failed to reject the null hypothesis, indicating a good fit

# 2. checking for mulitcollinearity using varience inflation factor
vif(glm.forward) # all the variables have a VIF of less than 2, there is no multicollinearity 

# 3 checking for possible outlier using Cook's distance
plot(glm.forward, which = 4) # Cook's distances are less than 1, hence there is no outlier
...
# the accuracy of the model is 86, therefore 86 percent of the time 
# --the model is predicting the correct value whether there is the presence or absence of disease

# False Negative
FN <- test[2,1]/sum(test) * 100
# roughly 7 percent of the time the model is predicting there is no heart disease while there is one